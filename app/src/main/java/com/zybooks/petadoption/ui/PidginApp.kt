...
fun WordCard(word: String, onButtonClick: () -> Unit, isSelected: Boolean){
   TextButton(onClick = onButtonClick){
      Row(modifier = Modifier.width(280.dp).background(color = Color.White).padding(15.dp).height(40.dp), horizontalArrangement = Arrangement.SpaceBetween){
         Text(word)
         Icon(imageVector = Icons.Filled.Star, contentDescription = "Dropdown", tint = if (isSelected) MaterialTheme.colorScheme.primary else Color.Gray)
      }
   }
